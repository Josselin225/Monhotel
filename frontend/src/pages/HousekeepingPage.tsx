...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: string }> = {
  pending:     { label: 'À nettoyer',  color: 'bg-red-100 text-red-700',    icon: 'bi-exclamation-circle' },
  in_progress: { label: 'En cours',   color: 'bg-amber-100 text-amber-700', icon: 'bi-arrow-repeat' },
  done:        { label: 'Terminé',    color: 'bg-blue-100 text-blue-700',   icon: 'bi-check-circle' },
  inspected:   { label: 'Inspecté',   color: 'bg-green-100 text-green-700', icon: 'bi-patch-check' },
}
    ...
    <div className="p-4">
      <PageHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-gray-700 font-medium">Gestion des tâches de nettoyage</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
        {[
          { label: 'À nettoyer',  value: stats?.pending ?? pending,     color: 'text-red-600',   icon: 'bi-exclamation-circle' },
          { label: 'En cours',    value: stats?.in_progress ?? in_progress, color: 'text-amber-600', icon: 'bi-arrow-repeat' },
          { label: 'Terminées',   value: stats?.done ?? done,           color: 'text-blue-600',  icon: 'bi-check-circle' },
          { label: 'Inspectées',  value: stats?.inspected ?? 0,         color: 'text-green-600', icon: 'bi-patch-check' },
        ].map(s => (
      ...
      <div className="card mb-4 flex flex-wrap gap-3 items-center">
        <div className="flex items-center gap-2 flex-1 min-w-[160px]">
          ...
          <input
            ...
            className="flex-1 text-sm placeholder:text-gray-400 bg-transparent focus:outline-none"
          />
        </div>
        <select
          value={filter.status}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          <i className="bi bi-check2-all text-5xl text-green-300" />
          <p className="mt-3 font-medium text-gray-500">Aucune tâche en cours</p>
            ...
            return (
              <div key={task.id} className="card hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="text-lg font-bold text-gray-900">Chambre {task.room_number}</p>
                    <p className="text-xs text-gray-500">{task.room_type} · Étage {task.floor}</p>
                  </div>
                ...
                {task.notes && (
                  <p className="text-xs text-gray-500 italic mt-1 mb-2 line-clamp-2">{task.notes}</p>
                )}
                ...
                <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                  ...
                  {task.status === 'done' && (
                    <button
                      ...
                      className="flex-1 text-xs py-1.5 justify-center bg-green-500 hover:bg-green-600 text-white rounded-lg flex items-center gap-1 transition-colors"
                    >
