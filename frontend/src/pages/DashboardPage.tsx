    ...
    <div className="p-4 space-y-4">
      <PageHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs text-gray-400 capitalize font-medium">{today}</p>
      ...
      {((notifications?.arrivals.length ?? 0) > 0 || (notifications?.departures.length ?? 0) > 0) && (
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          {(notifications?.arrivals.length ?? 0) > 0 && (
            <div className="card">
              <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <i className="bi bi-airplane-engines text-blue-500" />
                ...
                {notifications!.arrivals.map(b => (
                  <div key={b.id} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 font-mono">{b.reference}</p>
                    </div>
                    <button
          ...
          {(notifications?.departures.length ?? 0) > 0 && (
            <div className="card">
              <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <i className="bi bi-luggage text-orange-500" />
              ...
              <div className="space-y-2">
                {notifications!.departures.map(b => (
                  <div key={b.id} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                    <div className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center text-orange-600 font-bold text-sm shrink-0">
                      {b.room}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{b.client}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="card lg:col-span-2">
          <h3 className="font-semibold text-gray-900 mb-4">Revenus & occupation — 12 mois ({getCurrencyLabel()})</h3>
          {loading ? (
        ...
        {/* Statut chambres */}
        <div className="card">
          <h3 className="font-semibold text-gray-900 mb-4">État des chambres</h3>
      ...
      {!loading && (analytics?.monthly ?? []).length > 0 && (
        <div className="card">
          <h3 className="font-semibold text-gray-900 mb-4">Évolution taux d'occupation — 7 derniers mois</h3>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {[
            { href: '/app/bookings', label: 'Nouvelle réservation', icon: 'bi-plus-circle',  color: 'text-amber-600',  bg: 'bg-amber-50'  },
            { href: '/app/clients',  label: 'Nouveau client',       icon: 'bi-person-plus',  color: 'text-blue-600',   bg: 'bg-blue-50'   },
            { href: '/app/rooms',    label: 'Gérer les chambres',   icon: 'bi-door-open',    color: 'text-green-600',  bg: 'bg-green-50'  },
            { href: '/app/planning', label: 'Planning',             icon: 'bi-calendar3',    color: 'text-purple-600', bg: 'bg-purple-50' },
            { href: '/app/billing',  label: 'Facturation',          icon: 'bi-receipt',      color: 'text-orange-600', bg: 'bg-orange-50' },
          ].map(a => (
            <a key={a.href} href={a.href}
              className="flex flex-col items-center gap-2.5 p-4 rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all duration-200 text-center group bg-white"
            >
          ...
          <div className="modal-box max-w-md">
            <div className="modal-header">
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${quickModal === 'arrivals' ? 'bg-blue-100' : 'bg-orange-100'}`}>
                  <i className={`bi ${quickModal === 'arrivals' ? 'bi-airplane-engines text-blue-600' : 'bi-luggage text-orange-600'} text-lg`} />
                </div>
                <div>
              ...
              <button onClick={() => setQuickModal(null)} className="text-gray-400 hover:text-gray-600">
            ...
            <div className="p-4 space-y-2 max-h-80 overflow-y-auto">
              ...
              {(quickModal === 'arrivals' ? notifications?.arrivals : notifications?.departures)?.map(b => (
                <div key={b.id} className="flex items-center gap-3 py-2.5 px-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 ${quickModal === 'arrivals' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>
                    {b.room}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{b.client}</p>
